...
# Filter for rows that contain the Chainsaw ID
print(f"Filtering for Chainsaw ID: {TARGET_ID}...")
chainsaw_df = df[df['labels'].str.contains(TARGET_ID, na=False)]

total_available = len(chainsaw_df)
print(f"Found {total_available} potential segments.")
...
for index, row in chainsaw_df.iterrows():
    ...
    # Create the filename (e.g., chainsaw_dataset/VIDEOID.wav)
